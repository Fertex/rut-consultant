...
class WebActions:
    ...
    def get_judicial_orders(self, input_data: list):
        # Initializing patter constants for specific portal elements and output structure
        COURT_PATT_INDEX = '//li[@data-original-index="{}"]/a/span[@class="text"]'
        COURT_PATT_NAME = '//li[@data-original-index]/a/span[@class="text" and text()="{}"]'
        DOC_GET_URL = 'https://oficinajudicialvirtual.pjud.cl/ADIR_871/laboral/documentos/docuDemanda.php?dtaDoc='
        rut = input_data[0]
        output = None
        output_structure = {}

        # urls used in the process
        main_url = 'https://oficinajudicialvirtual.pjud.cl/home/index.php'
        data_url = 'https://oficinajudicialvirtual.pjud.cl/indexN.php'

        # Opening web portal with the session and getting to main data page
        self.session.get(main_url)
        self.session.find_element_by_xpath('//button[text()="Otros servicios"]').click()
        self.session.find_element_by_xpath('//div[button[@class="dropbtn"]]//a[text()="Consulta de Causas"]').click()

        # Waiting for the data page to be fully loaded
        data_page_loaded = False
        while not data_page_loaded:
            data_page_loaded = True if self.session.current_url == data_url else False
            sleep(2)

        # Filling the input form with request data
        self.session.find_element_by_xpath('//a[contains(text(), "Rut Persona")]').click()
        sleep(1)
        self.session.find_element_by_id('rutJur').send_keys(rut.split('-')[0])
        self.session.find_element_by_id('dvJur').send_keys(rut.split('-')[-1])
        self.session.find_element_by_xpath('//form[@id="formConsultaJur"]//option[@value="4"]').click()
        sleep(1.5)

        # Initializing main array for output data and starting main loop
        orders = []
        for i in range(2, 149+1):
            self.session.find_element_by_xpath('//button[@data-id="jurTribunal"]').click()
            sleep(0.5)

            if input_data[2]:
                # If specific court sent, select the court by name
                self.session.find_element_by_xpath(COURT_PATT_NAME.format(input_data[2])).click()

            else:
                # Else go searching every court by index
                self.session.find_element_by_xpath(COURT_PATT_INDEX.format(i)).click()

            self.session.find_element_by_css_selector('button#btnConConsultaJur').click()
            sleep(2.4)

            # Search for portal alerts and stop the process if needed
            ...
            # Search if data received for specific court, if empty skip court
            try:
                self.session.find_element_by_xpath('//tbody[@id="verDetalleJuridica"]/tr/td/button')
                continue

            except NoSuchElementException:
                pass

            # Looking for result quantity and start pagination if needed
            results_count = int(self.session.find_element_by_xpath('//div[@class="loadTotalJur"]/b').text)
            pages = ceil(results_count / 99)
            for page in range(pages):
                # Searching data in every possible page

                if page > 0:
                    # Change page number if there are no more data to read
                    next_btn = self.session.find_element_by_xpath('//ul[@class="pagination"]/li[@class="page-item"]/a')
                    self.session.execute_script("arguments[0].click();", next_btn)
                    sleep(2)

                # Detecting every table row on the page and looping over them
                rows = self.session.find_elements_by_xpath('//tbody[@id="verDetalleJuridica"]/tr[not(@style)]')
                for row in range(len(rows)):
                    data_element = self.session.find_elements_by_xpath(
                        f'//tbody[@id="verDetalleJuridica"]/tr[not(@style)][{row+1}]/td[not(@align)]')

                    if input_data[1]:
                        # If specific "estado de causa" sent them skip unnecessary data
                        if data_element[-1].text != input_data[1]:
                            continue

                    # Fill the output structure with row´s data as needed
                    output_structure['rit'] = data_element[0].text
                    output_structure['tribunal'] = data_element[1].text
                    output_structure['caratulado'] = data_element[2].text
                    output_structure['fechaIngreso'] = data_element[3].text
                    output_structure['estadoCausa'] = data_element[4].text

                    # If specific court sent, search the "documento de demanda" related to the row
                    if input_data[2]:
                        link = self.session.find_element_by_xpath(
                            f'//tbody[@id="verDetalleJuridica"]/tr[not(@style)][{row+1}]/td[@align]/a')
                        doc_id = self.get_order_document(link)  # Function used to obtain the document's link
                        output_structure['linkDocumentoDemanda'] = DOC_GET_URL + doc_id  # Adding the link to the output

                    # Add the row data to the orders array and clear the structure for incoming row
                    orders.append(output_structure)
                    output_structure = {}

            # If specific court sent, just search in one court then stop
            if input_data[2]:
                break

        # If there were no errors fill the output with the orders array and send the response
        if output is None:
            output = {"causas": orders}

        return output

    def get_order_document(self, link_element):
        # Assert that there are no pop-ups in the main page, close them if needed
        close_head = self.session.find_elements_by_xpath(
            '//div[@id="modalTextoDemandaLaboral" and contains(@style, "block")]//button[@class="close"]')
        if len(close_head) > 0:
            close_head[0].click()
            sleep(0.2)

        close_table = self.session.find_elements_by_xpath(
            '//div[@id="modalDetalleLaboral" and contains(@style, "block")]//button[@class="close"]')
        if len(close_table) > 0:
            close_table[0].click()
            sleep(0.2)

        # Open the detail order pop-up
        link_element.click()
        link = ''
        charging = True
        intent_counter = 0

        # Wait for the pop-up to fully charge
        while charging:
            try:
                self.session.find_element_by_xpath(
                    '//table[contains(@class, "table-titulos")]//td/a[contains(@href, "Demanda")]').click()
                charging = False
                sleep(1.7)
                # Get document row from the element attribute
                doc_link = self.session.find_element_by_xpath(
                    '//div[@id="modalTextoDemandaLaboral"]//tr[td/i[contains(@class, "check")]]/td//input')
                link = doc_link.get_attribute('value')
                # Close every pop-up
                self.session.find_element_by_xpath(
                    '//div[@id="modalTextoDemandaLaboral"]//button[@class="close"]').click()
                sleep(0.5)
                self.session.find_element_by_xpath('//div[@id="modalDetalleLaboral"]//button[@class="close"]').click()

            except (ElementNotInteractableException, NoSuchElementException, StaleElementReferenceException):
                # Try opening the detail pop-up up to 150 attempts, if still no answer then skip row
                intent_counter += 1
                if intent_counter >= 150:
                    sleep(1)
                    break
